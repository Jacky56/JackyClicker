public class CharacterPanel extends GeneralPanel{
  ...
  AutoConfigurePanel aCP;
  
  boolean AutoOn;
  
  long ChangeInLevel;
  
  public CharacterPanel(float x,float y,float sizeX,float sizeY,color background,float padding, float curve,color lineColor,float lineSize,AutoConfigurePanel aCP,long Level) {
    ...
    StatBody = new ScrollBar(585,y+sizeY - 170 - moveY,x+sizeX,15,8,sizeY,color(0,150),0,color(255),1);
  ...
  void ActionEvent(Player pl) {
    ...
    if(AutoOn) {
      if(ChangeInLevel != pl.Level) {
        for(int n = 0; n < aCP.statRatio.length; n++) {
          if(aCP.statRatio[n] != 0) {
            ApplyStat(n,pl,aCP.statRatio[n]);
          }
        }
        ChangeInLevel ++;
      }
  ...
  public void draw(Player pl) {
    ...
    if(show) {
      ...
      clip(x+padding,y+padding+19 + moveY,x+sizeX-padding,y+sizeY - 210 - moveY);
      
      TotalContent(pl,0,30 +StatBody.contentMove() + moveY);
      BaseContent(pl,0,105 +StatBody.contentMove() + moveY);
      ItemContent(pl,0,160 +StatBody.contentMove() + moveY);
      BonusContent(pl,0,215 +StatBody.contentMove() + moveY);
      TotalContentAdv(pl,0,270 +StatBody.contentMove() + moveY);
      BaseContentAdv(pl,0,360 +StatBody.contentMove() + moveY);
      ItemContentAdv(pl,0,425 +StatBody.contentMove() + moveY);
      BonusContentAdv(pl,0,490 +StatBody.contentMove() + moveY);
  ...
  void BaseContentAdv(Player pl,float cX,float cY) {
    fill(180);
    textSize(10);
    textAlign(TOP,TOP);
  ...
  void ItemContentAdv(Player pl,float cX,float cY) {
    fill(180);
    textSize(10);
    textAlign(TOP,TOP);
  ...
  void BonusContentAdv(Player pl,float cX,float cY) {
    fill(180);
    textSize(10);
    textAlign(TOP,TOP);
