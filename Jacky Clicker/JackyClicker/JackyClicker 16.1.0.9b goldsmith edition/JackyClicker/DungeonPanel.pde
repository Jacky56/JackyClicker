//..
protected class DungeonPanel extends GeneralPanel {
  //pointer  
  private Player pl;
  ...
  private ArrayList<AreaDungeon> ad;
  
  private ArrayList<Button> AreaButton = new ArrayList<Button>();
  
  
  private Button[] MileButton = new Button[8];
  private long[] MilePrice = new long[MileButton.length];
  private float[] MileStone = new float[MileButton.length];
  
  private int areaButtonClicked;
  
  
  //..  
  //ArrayList[] MileButtonShow = new ArrayList<Boolean>();
  protected DungeonPanel(Player pl,ArrayList<AreaDungeon> ad,float x,float y,float sizeX,float sizeY,color background,float padding, float curve,color lineColor,float lineSize) {
  ...
  //says what it does   
  private void createMileStoneButton() {
  ...
  private void movePlayerToDungeon(int n) {
  ...
  private boolean checkMileButtonPrice(int ButtonIndex,Player pl) {
  ...
  public void updateLineColorAreaButton(int i) {
    ...
    for(int n = 0; n < MileButton.length; n++) {
      ...
      MilePrice[n] = 500*n + (long)(n*pow((ad.get(i).MileMax/1000)*16,pow(n,0.5))); 
