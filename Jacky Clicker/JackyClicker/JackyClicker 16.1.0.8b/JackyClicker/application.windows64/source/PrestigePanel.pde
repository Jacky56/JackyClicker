public class PrestigePanel extends GeneralPanel {
  ...
  public PrestigePanel(Player pl,float x,float y,float sizeX,float sizeY,color background,float padding, float curve,color lineColor,float lineSize) {
  ...
  float correctUpgradeButtonX() {
    return x+padding + (45+2)*(UpgradeButton.size() % 4);
  }
  float correctUpgradeButtonY() {
  ...
  boolean checkUpgradeButtonPrice(int ButtonIndex) {
