...
void setup() {
  ...
  player = new Player("Player","This is the Player of the game",width/2 -40,height/2 +80,100,90,80,70,60);
  enemy = new Enemy("Enemy","Click on this to do damange",width/2 -100,height/2 - 200,100,90,80,70,60,0.5,10,10);
...
void draw() {
  ...
  for(int n = 0; n < goldenCookie.size();n++) {
    goldenCookie.get(n).GoldenCookieAsButton.hoverInfo.draw();
  }  
  ...
  for(int n = 0; n < panelParty.PPP.size();n++) {
    for(int i = 0; i < panelParty.PPP.get(n).buffIcon.size();i++) {
      if (n < 11) {
        panelParty.PPP.get(n).buffIcon.get(i).BuffInfo.draw();
      }
    }
  }  
