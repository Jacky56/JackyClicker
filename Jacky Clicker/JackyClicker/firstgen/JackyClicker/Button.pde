...
public class Button implements name{
  float x,y;
  float sizeX,sizeY;
  String title;
  float textSize;
  ...
  public Button(String title,String infoDescription,float x,float y,float sizeX,float sizeY,color background,color textColor,float textSize) {
    ...
    test = new HoverInfo(title,infoDescription,14,color(255),175,7,color(25,25,200,150),x,y,sizeX,sizeY,5,2,color(255));
  }
  ...
  //pure custom button
  public Button(String text,String infoDescription,float x,float y,float sizeX,float sizeY,float curve0,float curve1,float curve2,float curve3,color background,color textColor,float textSize,color lineColor,float lineSize) {
    this(text,infoDescription,x,y,sizeX,sizeY,background,textColor,textSize);
    this.lineColor = lineColor;
    this.lineSize = lineSize;
    this.curve[0] = curve0;
  ...
  void draw() {
    ...
    fill(textColor);
    textSize(textSize);
    text(title,x+sizeX/2,y+sizeY/2);
    if(mouseOn()) {
      cursor(HAND);
    }
    //temp
