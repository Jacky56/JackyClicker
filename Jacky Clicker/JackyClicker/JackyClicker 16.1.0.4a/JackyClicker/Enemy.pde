public class Enemy extends Unit {
  ...
  public Enemy(String Name,String Description,float x,float y,long CharVit,long CharStr,long CharEnd,long CharAgi,long CharDex,double AttackTimer,long ExpBounty,long GoldBounty,long PrestigeBounty) {
  ...
  public void Attack(Player pl) {
    ...
    if(CurrentAttackTimer > AttackTimer && pl.Alive && Alive) {
      ...
      if(TotalDmg > pl.TotalDef) {
        pl.CurrentHp -= (TotalDmg-pl.TotalDef);
        //(String text,float speedY,float LifeTime,float x,float y,float textSize,color textColor)
        //temp
        if(pl.ThisPlayer) {
          popText.add(new PopText((long)(TotalDmg-pl.TotalDef)+"",1,0.5,pl.x + pl.UnitAsButton.sizeX/2,pl.y,30,color(200,25,25)));
        }
      }
    }
    ...
    if(UnitPlaced) {
      if(Alive == false && GaveBounty == false) {
        ...
        for(int n = 0; n < pl.size();n++) {
          Player pla = pl.get(n);
          long exp = (long)(ExpBounty*(pla.CharExp + pla.ItemExp + pla.BonusExp));
        ...
        if(PrestigeBounty != 0) {
          popText.add(new PopText("+"+(long)(PrestigeBounty*(ThisPlayer.CharPrestige + ThisPlayer.ItemPrestige + ThisPlayer.BonusPrestige))+" favor",1,random(1,1.5),random(x+UnitAsButton.sizeX/4,x+UnitAsButton.sizeX*3/4),random(y+UnitAsButton.sizeY/4,y+UnitAsButton.sizeY*3/4),20,color(218,112,214)));
        }
        
        if(ExpBounty != 0) {
          popText.add(new PopText("+" +(long)(ExpBounty*(ThisPlayer.CharExp + ThisPlayer.ItemExp + ThisPlayer.BonusExp))+" exp",1,random(1,1.5),random(x+UnitAsButton.sizeX/4,x+UnitAsButton.sizeX*3/4),random(y+UnitAsButton.sizeY/4,y+UnitAsButton.sizeY*3/4),20,color(255,255,51)));
        }
      }
    }
  }
