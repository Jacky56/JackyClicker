protected class RecruitConfigurePanel extends GeneralPanel {
  
  private ScrollBar RecruitBody;
  private float moveY = 30;
  
  private Button PayForAll;
  
  private Button Close;
  
  private Button OverWorkButton;
  
  private boolean OverWorkOn;
  
  
  private double TotalDps;
  
  protected RecruitConfigurePanel(float x,float y,float sizeX,float sizeY,color background,float padding, float curve,color lineColor,float lineSize) {
  ...
  protected void ActionEvent(Player pl, RecruitPanel rP) {
    if(show) {
  ...
  private void checkPayAllButton(Player pl) {
    for(int n = 0; n < pl.guildUnit.size(); n++) {
  ...
  private void checkPayButton(GuildUnit gu,Player pl) {
  ...
  private boolean checkPayButtonPrice(GuildUnit gu,Player pl) {
  ...
  private void RecruitInfo(GuildUnit Gu,float cX,float cY) {
