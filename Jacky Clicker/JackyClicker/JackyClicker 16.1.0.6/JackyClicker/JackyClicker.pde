...
void TimeEvent() {
  ...
  //temp
  for(int n = 0; n < aPlayer.size();  n++) {
    if(aPlayer.get(n).LvlUp.size() > 10) {
      aPlayer.get(n).LvlUp.remove(0);
    }
  }
  
  //temp
  for(int n = 0; n < aPlayer.size();  n++) {
    for(int i = 0; i < aPlayer.get(n).LvlUp.size();i++) {
      aPlayer.get(n).LvlUp.get(i).TimeEvent();
      if(aPlayer.get(n).LvlUp.get(i).Active == false) {
        aPlayer.get(n).LvlUp.remove(i);
      }
    }
  }
...
void ActionEvent() {
  ...
  leftTabUI.ActionEvent(aPlayer.get(1));
  ...
  if(enemy.Alive == false ) {
    killcount++;
    float m = pow(1.03,killcount);
    
    for(int n = 0; n < aPlayer.size(); n ++){
      for(int i = 0; i < aPlayer.get(n).guildUnit.size(); i++) {
        aPlayer.get(n).guildUnit.get(i).TriggerOnEnemyDeadEvent(enemy);
      }
    }
...
void draw() {
  ...
  for(int n = 0; n < aPlayer.get(1).LvlUp.size(); n++) {
    aPlayer.get(1).LvlUp.get(n).draw();
  }
